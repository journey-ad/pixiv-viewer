<template>
  <div class="artwork">
    <TopBar />
    <swiper class="artwork-swiper" :options="swiperOptions" ref="gallery">
      <!-- <swiper-slide v-for="artwork in galleryList" :key="artwork.id">
        <ImageView :artwork="artwork" />
        <Meta :artwork="artwork" />
        <van-divider />
        <AuthorCard v-if="artwork.author" :id="artwork.author.id" />
      </swiper-slide>-->
      <swiper-slide v-if="galleryList[currentIndex-1]">
      ...
      <swiper-slide v-if="galleryList[currentIndex]">
